					...
					<i @click="toggleTranslator" v-if="notTyping" class="input-icons-style" title="Switch Translator">
						...
						>
							<path
								...
								d="M19.5 12c0-1.232-.046-2.453-.138-3.662a4.006 4.006 0 00-3.7-3.7 48.678 48.678 
...
<script setup>
	import { inject, ref, watchEffect, defineAsyncComponent, provide } from "vue";
		...
		try {
			if (input.value.length >= 2) {
				switch (placeholder.value) {
					case text1:
						emitter.emit("use-input-igbo", input.value); // 👉search
					...
					case text2:
						emitter.emit("use-input-english", input.value);
