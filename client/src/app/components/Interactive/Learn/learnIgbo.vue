<template>
	<div class="h-screen overflow-hidden overflow-x-hidden bg-gray-100 select-none dark:bg-slate-900 font-body">
		<Header
			@toggl-theme="setDarkMode"
			class="z-10"
			title="Search Words"
			:dark-state="darkState"
			:account-state="authService.currentUserValue.user.email"
		/>
		<Transition>
			<Home @alpha="toggleView('alpha')" @number="toggleView('number')" @vowels="toggleView('vowels')" :use-home="useHome" />
		</Transition>
		<Transition>
