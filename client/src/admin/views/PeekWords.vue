<template>
  <div class="absolute z-20 flex flex-col items-center w-full h-full gap-6 p-4 md:w-4/5 font-body">
    <span class="flex justify-between w-full overflow-hidden rounded-md mt-14 md:w-1/2 bg-slate-300 dark:bg-slate-600">
      <div v-if="dropDownRecords" class="use_dropdown_peek_left">
        ...
        <button @click="dispatch('allRecords')" class="border-t use_buttons border-slate-500 dark:border-slate-400"
      ...
      <button @click="fetchRecords(constant)"
      ...
      <button @click="fetchQuizzes(quizConstant)"
      ...
      <div @click="dropDownQuizzes = !dropDownQuizzes" class="peek_dropdown" title="Selects the quiz category to view">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-blue-700 dark:hover:text-blue-500" fill="none"
          viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
        </svg>
    ...
    <div class="flex flex-row gap-4 p-2">
      <div class="flex flex-row flex-wrap w-full gap-1 p-1 border border-gray-200 rounded-md dark:border-slate-500">
        <li class="tags" v-if="useWords" v-for="{ name, translation, genre, _id } in allWords" :key="_id">
          {{ name }} - {{ translation }} @ {{ genre }}
        </li>
        <li class="tags" v-if="useRecords" v-for="{ name, translation: [trans], genre, definitions } in allDict"
          :key="_id">
...
async function fetchQuizzes(factor) {
  ...
  switch (factor) {
    case "search":
      return fetchQuiz(quizServe, quizOk, quizFail, allQuizzes, factor)
    case "dict":
      return fetchQuiz(quizServe, quizOk, quizFail, allQuizzes, factor)
    default:
      flashError()
