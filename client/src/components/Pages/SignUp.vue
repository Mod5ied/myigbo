<template>
    <form @submit.prevent=""
        class="flex flex-col items-center justify-around w-3/4 p-2 border bg-gray-50 rounded-md h-72">
        <span class="flex flex-col justify-between gap-1 w-3/4">
        ...
        <span class="flex flex-col justify-between gap-1 w-3/4">
            <label for="password" class="w-2/5 text-sm font-semibold text-slate-800">Confirm Password</label>
        ...
        <span v-if="isAdmin" class="flex flex-col justify-between gap-1 w-3/4">
        ...
        <button :disabled="hasError" type="submit"
            class="text-white p-2 bg-slate-900 hover:bg-slate-700 rounded-lg text-sm font-semibold w-3/4">{{ hasError ?
