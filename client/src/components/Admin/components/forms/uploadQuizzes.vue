<template>
  ...
  <form @submit.prevent="handleSubmit(quizConstant)"
    ...
    <div
      ...
      <input @change="fail_upload = false" type="text" id="igbo" name="igbo" :disabled="quizConstant == 'search'"
        :placeholder="quizConstant == 'search' ? 'not required' : 'Enter another wrong answer'" v-model="wrong_answer2"
        class="dark_inputs" />
    </div>
    <!-- submitting section 👇 -->
