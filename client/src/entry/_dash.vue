<template>
	<div class="flex items-center justify-center w-screen h-screen">
		...
		<div class="flex items-center justify-center w-screen h-screen opacity-75 bg-slate-200 dark:opacity-40 dark:bg-slate-900"></div>
		<div class="registrationFormWrapper">
			<div class="flex flex-row justify-around w-4/5 border md:w-1/3 border-slate-400">
				<p
					...
					class="w-1/2 p-2 text-center border-r border-r-slate-400"
				>
					Log-In
				</p>
				<p
			...
			<transition-group name="my-animation">
				<span v-if="showError" class="absolute flex items-center justify-center w-4/6 h-16 ml-0 mr-0 -mt-8 bg-red-400 rounded-md">
					<p class="text-center text-gray-100">{{ errorMessage }}</p>
				</span>
			</transition-group>
			<!-- prompt ui here. -->
			<transition-group name="my-animation">
				<span v-if="showPrompt" class="absolute flex items-center justify-center w-4/6 h-16 ml-0 mr-0 -mt-8 bg-green-700 rounded-md">
			...
			<span class="relative flex items-center border dark:border-dotted md:right-40">
				<router-link class="p-1 text-sm font-semibold text-indigo-900" :to="{ name: 'Search' }">Continue without Sign-Up</router-link>
...
<script setup>
	import { ref, inject, defineAsyncComponent, onBeforeMount } from "vue";
	...
	async function userLookup() {
		const currentUser = await authService.currentUserValue;
