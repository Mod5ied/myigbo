...
async function login(email, password, path, role) {
	if (currentUserSub.value) return responseHandler({ status: 204, message: "token exists", token: currentUserSub.value.token });
...
async function signout(email, password, path, role) {
	...
	try {
		const res = await axios.delete(authRoute.url(role, path), data);
		const result = (await res.data) && localStorage.removeItem("currentUser");
		return responseHandler(result);
	} catch (err) {
