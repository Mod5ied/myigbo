...
class Users {
    ...
    /* redundancies, fix this with classes. */
    if (req.payload == null) return next(ApiError.badRequest("User role is not defined"));
    ...
    if (req.payload == null) return next(ApiError.badRequest("User role is not defined"));
		...
		return res.status(200).json({ message: `About ${count} active user(s) found`, users });
		}
		return next(ApiError.unauthorizedRequest("Request is unauthorized"));
	};
